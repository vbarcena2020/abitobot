...
def generate_launch_description():
    # moveit_config = MoveItConfigsBuilder("rover", package_name="abitobot_moviet_config").to_moveit_configs()
    ...
    pkg_path = get_package_share_directory('abitobot')
    ...
    model_path = get_model_paths(['abitobot'])

    robot_description_launcher = IncludeLaunchDescription(
       PathJoinSubstitution(
           [FindPackageShare("abitobot_moveit_config"), "launch", "rsp.launch.py"]
       ),
    )
    ...
    rviz_config_file = PathJoinSubstitution(
        [FindPackageShare("abitobot"), "rviz", "robot.rviz"]
    )
    ...
    bridge = Node(
        ...
        parameters=[
            {
                'config_file': join(
                    pkg_path, 'config', 'abitobot_bridge.yaml'
                ),
                'use_sim_time': True,
            }
        ],
